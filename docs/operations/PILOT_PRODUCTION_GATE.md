...
- **Auth0:** confirm the AU tenant and plan support New Universal Login passwordless email OTP, the production custom domain, sender constraints, and required features. Confirm the live entitlement separately; public pricing is not entitlement evidence. If a custom domain is approved, complete Auth0's required credit-card verification without recording card data in TGE evidence. Classic Login and magic links are not accepted Pilot fallbacks.
- **Auth0 operator provisioning:** approve a dedicated machine-to-machine client limited to `read:users` and `create:users`, enable only the intended passwordless email connection, and provide the exact issuer and same-origin `/api/v2/` Management API base through the [identity operator runbook](IDENTITY_OPERATOR_RUNBOOK.md). Do not grant update/delete-user or tenant-administration scopes. Record secrets only in the approved secret system, never in command lines, logs, tickets, or screenshots.
