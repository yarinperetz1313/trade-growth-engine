...
`src/auth/postgresAuthRepository.js` is the PR-3 integration seam. PR-4 does not redesign or claim the unavailable PR-3 CRM repository implementation. When auth mode is enabled without an explicit tenant-persistence boundary, legacy business APIs return `503 TENANT_PERSISTENCE_UNAVAILABLE` rather than exposing unscoped JSON data.
