...
The server-only Auth0 provisioning adapter requires one exact issuer, same-origin Management API base, passwordless email connection, and injected access-token provider. Every lookup/create/conflict success proves exactly one Auth0 identity has that exact connection, the `email` provider, and a provider/user identifier that reconstructs the authoritative subject exactly. It creates only when no user exists, fails closed on missing, linked, wrong, conflicting, ambiguous, provider, or configuration evidence, and never logs provider responses or credentials. Apply validates all configuration and exclusively reserves the empty `0600` capability output before a read-only privileged database/OWNER preflight; provider access follows only after that succeeds. The later operator transaction repeats authority checks and creates the pending invitation with the provider-returned exact issuer and subject already present. It writes the random invitation capability only to the reserved file and persists only its SHA-256 hash. Retry uses the same operation UUID and exact facts; conflicting reuse fails closed.
...
The invitation is read only from a bounded fragment route. A valid invitation landing takes precedence over ordinary membership resolution even when an Auth0 session already exists, so an authenticated but not-yet-activated invitee can explicitly start the intended flow. Passive landing performs no context, invitation-begin, or provider action. An explicit continue action proves server availability, then Auth0 receives only the bounded invitation token and return route in SDK `appState`. After the exact callback is consumed and scrubbed, the browser sends a fresh in-memory access token to invitation acceptance before requesting membership-derived context. It renders the authenticated application only after both operations succeed; issuer/subject matching remains exclusively server-side.

Returning users have an explicit login action and authenticated users have an explicit logout action. Uninvited, wrong-user, expired, revoked, and replayed cases share generic unavailable presentation. A malformed invitation token in consumed callback `appState` instructs the user to restart from the original invitation; consumed callback state with no usable `appState` truthfully returns the user to sign in instead of silently attempting membership resolution. Neither recovery nor navigation reads tenant/role claims, persists access tokens, offers signup, or chooses a tenant.
