...
- RevenueLeakCase stores immutable, tenant-scoped leak evidence and deterministic
  ...
  Revenue Command Center V2 consumes its complete bounded operating queue and
  composes a safe handoff into the existing RevenueAction authority; Opportunity
  Command Center remains the detailed review/execution surface. No detector
...
optional same-opportunity RevenueAction linkage. It also owns the pure versioned
...
The browser uses that seam only through `web/lib/api.js`; Revenue Command Center
validates and renders the server-ordered queue, while Opportunity Command Center
renders per-opportunity detector outcomes and durable case history and its
existing RevenueAction panel remains the sole execution UI.
