...
_Last verified during harness implementation._
...
The main closed-loop feature is the Opportunity Command Center. It loads opportunities, opens an exact opportunity via `#opportunities/:id`, fetches deterministic deal intelligence, performs action mutations, and refreshes UI state from API responses.
...
- API/integration coverage is in `test/intelligence-api.test.js`.
- Browser E2E coverage is in `test/e2e/opportunity-command-center.spec.js`.
