...
create function tge.guard_runtime_revenue_action_lifecycle()
returns trigger
language plpgsql
...
begin
  ...
  if old.status = 'RECOMMENDED' and new.status = 'PREPARED' then
  ...
  elsif old.status in ('APPROVED', 'EXECUTING', 'FAILED')
    ...
    if new_audit_length <> old_audit_length + 1
      ...
      or new.execution_result->>'mode' is distinct from expected_mode
      or coalesce(btrim(new.execution_result->>'outcome'), '') in ('', 'FAILED')
      or new.execution_result->>'external_send_performed' is distinct from 'false'
      ...
      or (new.execution_type = 'INTERNAL_TASK' and new.resulting_task_id is null)
      or (new.execution_type = 'COMMUNICATION_DRAFT' and new.resulting_task_id is not null)
      or new.failed_at is not null then
      coherent := false;
    end if;
...
create trigger revenue_actions_runtime_lifecycle_integrity
before update on tge.revenue_actions
for each row execute function tge.guard_runtime_revenue_action_lifecycle();
