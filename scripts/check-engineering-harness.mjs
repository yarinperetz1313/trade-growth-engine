...
function validateDatabaseFoundationContract() {
  ...
  const authMigration = readFile(
    "database/migrations/005_auth_membership_and_invitations.sql"
  );
  ...
  requireText(
    ...
    "Migration 005 membership lookup must bind issuer and subject"
  );
  requireText(
    authMigration,
    "create table tge.assisted_invitations",
    "Migration 005 must add assisted invitation storage"
  );
  requireText(
    authMigration,
    "create function tge.consume_assisted_invitation",
    "Migration 005 must provide atomic invitation consumption"
  );
  requireText(
    databaseTest,
    "TGE_TEST_DATABASE_URL",
...
function validatePilotReadinessContract() {
  ...
  requireText(
    executionPlansIndex,
    "PR-3 is unavailable/not implemented",
    "Execution-plan index must mark Pilot PR-3 unavailable and not implemented"
  );
  requireText(
    plan,
  ...
  requireText(
    ...
    "Pilot plan must mark PR-3 unavailable and not implemented"
  );
  requireText(
    plan,
    "PR-4 — code and deterministic contracts implemented",
    "Pilot plan must mark PR-4 code implemented"
  );
